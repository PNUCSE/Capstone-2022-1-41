...
def camStart(previewName, camID, camFourCC):
    ap_model = SingleImageAlphaPose(args, cfg)
    model_path = '/home/safetylab/AlphaPose/alphapose/utils/ESPCN_x4.pb'
    sr = cv2.dnn_superres.DnnSuperResImpl_create()
    sr.readModel(model_path)
    sr.setModel("espcn", 4)
    im_name = "None"  # the path to the target image
    ...
    while rval:
        ...
        pose = ap_model.process(im_name, frame)
        img = ap_model.getImg()  # or you can just use: img = cv2.imread(image)
        img = ap_model.vis(img, pose)  # visulize the pose result
...
if __name__ == "__main__":
    camStart("thermal detection", "/dev/video2", cv2.VideoWriter.fourcc('Y','1','6',' '))
